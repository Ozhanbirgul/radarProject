...
const Modal = ({detailId, closeModal}) => {
  ...
  useEffect(() => {
    ...
    // bu sayede yükleniyor tetiklenecek
    setData(null);
 ...
 return moment(date).calendar();
  ...
  return (
    <div className="detail-outer">
      ...
      <p>
        <span>Kalkış: </span>
        <a target="b_blank" href={d.airport.origin.website} > {d.airport.origin.name} </a>
      </p>

      <p>
        <span>Hedef: </span>
        <a target="b_blank" href={d.airport.destination.name} > {d.airport.destination.name} </a>
      </p>
